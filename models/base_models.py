...
def make_layers(block, inplanes, planes, blocks, stride=1, dilation=1, padding=1):
    downsample = None
    ...
    layers = []
    bbl=block(inplanes, planes, stride, dilation, padding, downsample)
...
def resnet(cfg,in_channel=3):
    ...
    layers += make_layers(block,256*block.expansion,512,cfg[3], stride=1, dilation=2, padding=2)
    return layers
