...
class SSD(nn.Module):
    ...
    def __init__(self, base, head, num_classes,size):
...
def build_net(size=300, num_classes=21):
    if size != 300 and size != 512:
        ...
        return
    add_extras_list =[]
    return SSD(*multibox(vgg(vgg_base[str(size)], 3),mbox[str(size)], num_classes), num_classes=num_classes,size=size)
