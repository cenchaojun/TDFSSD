...
parser.add_argument('-th', '--threshold', default=0.3,
...
def slice_image(net, detector, transform, img, save_dir):
...
if __name__ == '__main__':
    ...
    result_path = '/home/cen/PycharmProjects/TDFSSD/eval_large'
        ...
        if rownum <= h and colnum <= w:
            ...
            for r in range(rowheight):
                for c in range(colwidth):
                    ...
                    box_list.append(box)
                    croped_image = img[Ly:Ry, Lx:Rx]
                    total_number, c_bboxes,c_dets =slice_image(net, detector, transform, croped_image, result_file)
                    print(c_dets)
                    print("##########################")
            ...
            keep2 = nms_py(dets=sum_cbbox,thresh=0.3)
            print(keep2)
            print(len(keep2))
