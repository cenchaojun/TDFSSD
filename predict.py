...
from data import AnnotationTransform, COCODetection, VOCDetection, BaseTransform, VOC_300,VOC_512,COCO_300,COCO_512,VOC_short_512
...
def test_net(net,img,name,detector,transform,priors,top_k=200,thresh=0.01):
    ...
    for j in range(1, 2):
        ...
        keep = nms(c_dets, 0.3, force_cpu=True)
        c_dets = c_dets[keep, :]
...
if __name__ == "__main__":
    Image = os.listdir('/home/cen/PycharmProjects/TDFSSD/testdata/')

    for img_name in Image:
        img = cv2.imread("/home/cen/PycharmProjects/TDFSSD/testdata/"+img_name)
        model = '/home/cen/PycharmProjects/TDFSSD/weights/sssd_512/0318/sssd_VOC_epoches_146.pth'
        net = build_net(512, 2)
        ...
        detector = Detect(2, 0, VOC_short_512)
        priorbox = PriorBox(VOC_short_512)
        # with torch.no_grad():
        #     priors = priorbox.forward()
        #     priors = priors.cuda()
        priors = Variable(priorbox.forward(), volatile=True)
        priors = priors.cuda()
        test_net(net, img, img_name, detector, transform, priors,top_k=200, thresh=0.5)